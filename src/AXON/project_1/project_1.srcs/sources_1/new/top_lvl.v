...
module top_lvl #(
    ...
    // Control
    input wire en_cntr,
    input wire [Dimension*Dimension - 1 : 0] en_in, //Enable inputs on diagonals? // What was on your mind?
    ...
    // en is singular for ALL register. Just
        ...
        for (i = 0; i < Dimension-1; i = i + 1) begin : GEN_UPPER_ROW
            for (j = i+1; j < Dimension; j = j + 1) begin : GEN_UPPER_COL
                PE_H #(.DW(DW)) pe_h (
                    ...
                    // From south
                    .weight_in(
                        weight_wires[i+1][j] // Updated. Except for diagonals, each PE recieve weights and inputs from neighboring PEs.
                    ),
