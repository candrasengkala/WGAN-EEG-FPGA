...
// axis_custom_top.v
// Verilog-2001 (IEEE 1364-2001) -- NO SystemVerilog constructs new
//
// FIX: Added ENABLE_S2MM parameter, propagated to MM2S_S2MM.
// Set ENABLE_S2MM=0 for write-only wrappers (e.g. bias_wrapper) to
// suppress S2MM FIFO instantiation and eliminate RTSTAT #1 warning.
...
module axis_custom_top #(
    ...
    wire                  wr_counter_enable, wr_counter_start;
    wire                  rd_counter_enable, rd_counter_start;
    wire [15:0]           wr_counter, rd_counter;
    wire [15:0]           wr_start_addr, wr_count_limit;
    wire [15:0]           rd_start_addr, rd_count_limit;
    ...
    always @(posedge aclk) begin
        if (!aresetn) begin
            ...
            sending_header        <= 1'b0;
            header_sent           <= 1'b0;
            auto_read_active      <= 1'b0;
            is_notification_mode  <= 1'b0;
            latched_rd_bram_start <= 3'd0;
            latched_rd_bram_end   <= 3'd0;
            latched_rd_addr_count <= 16'd0;
        end else begin
            if (send_header && !sending_header && !header_sent) begin
                ...
                sending_header        <= 1'b1;
                header_word_count     <= 3'd0;
                auto_read_active      <= 1'b1;
            end

            if (sending_header && s2mm_tready) begin
                if (header_word_count < 3'd5)
                    header_word_count <= header_word_count + 3'd1;
                else begin
                    sending_header <= 1'b0;
                    header_sent    <= 1'b1;
                end
    ...
    wire [2:0] rd_bram_start_to_fsm;
    wire [2:0] rd_bram_end_to_fsm;
    wire [15:0] rd_addr_count_to_fsm;
    wire        fsm_needs_trigger;
    wire        fsm_trigger_enable;

    assign use_auto_mode         = auto_read_active;
    assign instruction_to_fsm   = (use_auto_mode && header_sent && !is_notification_mode)
                                   ? 8'h02 : Instruction_code;
    assign rd_bram_start_to_fsm = use_auto_mode ? latched_rd_bram_start : rd_bram_start;
    assign rd_bram_end_to_fsm   = use_auto_mode ? latched_rd_bram_end   : rd_bram_end;
    assign rd_addr_count_to_fsm = use_auto_mode ? latched_rd_addr_count : rd_addr_count;
    assign fsm_needs_trigger     = use_auto_mode && header_sent && !is_notification_mode;
    assign fsm_trigger_enable    = bram_wr_enable_original || fsm_needs_trigger;
    ...
    // ========================================================================
    // MM2S_S2MM -- ENABLE_S2MM propagated from parent
    // ========================================================================
    MM2S_S2MM #(
        .FIFO_DEPTH  (512),
    ...
    demux1to16 #(.DATA_WIDTH(DATA_WIDTH)) demux_inst (
        .data_in (mm2s_tdata),
        .sel     (demux_sel[3:0]),
        .out_0   (demux_out[0]),  .out_1  (demux_out[1]),
        .out_2   (demux_out[2]),  .out_3  (demux_out[3]),
        .out_4   (demux_out[4]),  .out_5  (demux_out[5]),
        .out_6   (demux_out[6]),  .out_7  (demux_out[7]),
        .out_8   (demux_out[8]),  .out_9  (demux_out[9]),
        .out_10  (demux_out[10]), .out_11 (demux_out[11]),
        .out_12  (demux_out[12]), .out_13 (demux_out[13]),
        .out_14  (demux_out[14]), .out_15 (demux_out[15])
    );

    mux8to1 #(.DATA_WIDTH(DATA_WIDTH)) mux_inst (
        .in_0    (bram_dout[0]), .in_1 (bram_dout[1]),
        .in_2    (bram_dout[2]), .in_3 (bram_dout[3]),
        .in_4    (bram_dout[4]), .in_5 (bram_dout[5]),
        .in_6    (bram_dout[6]), .in_7 (bram_dout[7]),
        .sel     (mux_sel[2:0]),
        .data_out(mux_out)
    );
    ...
    // ========================================================================
    wire is_last_bram;
    wire auto_read_tlast;
    wire normal_read_tlast;
    wire notification_tlast;

    assign is_last_bram      = (mux_sel >= latched_rd_bram_end);
    assign auto_read_tlast   = rd_counter_done && is_last_bram &&
                               header_sent && !is_notification_mode;
    assign normal_read_tlast = rd_counter_done && !use_auto_mode;
    assign notification_tlast = sending_header &&
                                (header_word_count == 3'd5) &&
                                is_notification_mode;
    ...
    // ========================================================================
    // BRAM READ LATENCY COMPENSATION (1-cycle pipeline)
    // ========================================================================
    reg read_valid_pipe;
    always @(posedge aclk) begin
        if (!aresetn)
